...
export async function executeEmailAutomation(
  ...
  try {
    ...
    if (error) {
      ...
      // Check if it's a Resend domain/recipient restriction error
      if (errorMessage.includes("domain") || errorMessage.includes("verified") || errorMessage.includes("recipient")) {
        throw new Error(
          `${errorMessage}. Para enviar correos a cualquier cliente, necesitas verificar tu dominio en Resend. Consulta RESEND_SETUP.md para más información.`
        );
      }
    ...
    if (!data || !data.success) {
      ...
      // Check if it's a Resend domain/recipient restriction error
      if (errorMsg.includes("domain") || errorMsg.includes("verified") || errorMsg.includes("recipient")) {
        throw new Error(
          `${errorMsg}. Para enviar correos a cualquier cliente, necesitas verificar tu dominio en Resend. Consulta RESEND_SETUP.md para más información.`
        );
      }
...
export async function executeMeetingFollowUpAutomation(
  ...
  try {
    ...
    if (error) {
      const errorMessage = error.message || "Failed to send meeting follow-up email";
      // Check if it's a Resend domain/recipient restriction error
      if (errorMessage.includes("domain") || errorMessage.includes("verified") || errorMessage.includes("recipient")) {
        throw new Error(
          `${errorMessage}. Para enviar correos a cualquier cliente, necesitas verificar tu dominio en Resend. Consulta RESEND_SETUP.md para más información.`
    ...
    if (!data || !data.success) {
      const errorMsg = data?.error || "Failed to send meeting follow-up email";
      // Check if it's a Resend domain/recipient restriction error
      if (errorMsg.includes("domain") || errorMsg.includes("verified") || errorMsg.includes("recipient")) {
        throw new Error(
          `${errorMsg}. Para enviar correos a cualquier cliente, necesitas verificar tu dominio en Resend. Consulta RESEND_SETUP.md para más información.`
